...
class Post extends User{
	...
	public static function getPostLinks($post){
		...
		$post = preg_replace("/#([\w]+)/", "<a href='http://localhost/sosmed/hashtag.php?hashtag=$1'>$0</a>", $post);	
        
		$post = preg_replace("/@([\w]+)/", "<a href='http://localhost/sosmed/profile.php?username=$1'>$0</a>", $post);
		return $post;		
	}
